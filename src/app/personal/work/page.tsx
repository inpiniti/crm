...
import { listTaskOptions, listWorkByDate, listWorkDates, type WorkTimelineItem } from "@/infrastructure/supabase/repositories/tasks";
import { attempt } from "@/lib/attempt";
import { formatDateTime, todayKst, weekdayKst } from "@/lib/date";

export default async function PersonalWorkPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [rd, ro] = await Promise.all([
    attempt(() => listWorkDates(undefined, "personal")),
    attempt(() => listTaskOptions({ scope: "personal" })),
  ]);
  if (!rd.ok) return <SetupNotice error={rd.error} />;
  const dates = rd.value;
  const taskOptions = ro.ok ? ro.value : [];
  const date = requested ?? dates[0]?.date ?? today;

  const r = await attempt(() => listWorkByDate(date, "personal"));
  ...
  return (
    <div className="flex h-full w-full min-w-0 overflow-hidden">
      ...
      <WorkDateNav dates={dates} currentDate={date} today={today} taskOptions={taskOptions} basePath="/personal" title="개인 작업" />
      <section className="flex-1 h-full min-w-0 flex flex-col overflow-hidden bg-background">
        <DetailTopBar moduleName="개인 작업" subtitle={date} />
        <div className="flex-1 overflow-y-auto">
          <div className="mx-auto max-w-4xl p-6 sm:p-8 space-y-7">
            <div className="flex items-center justify-between border-b border-border/80 pb-5">
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1">
                  {prevDate ? (
                    <Link href={`/personal/work?date=${prevDate}`} className={navBtn} aria-label="이전 작업일">
                  ...
                  {nextDate ? (
                    <Link href={`/personal/work?date=${nextDate}`} className={navBtn} aria-label="다음 작업일">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {date === today && (
                      <span className="rounded-md bg-blue-weak px-2 py-0.5 text-[12px] font-semibold text-blue">오늘</span>
                    )}
                  </div>
                  {items.length > 0 && (
            ...
            {items.length === 0 ? (
              <EmptyState icon={Clock} title="이 날은 기록된 작업이 없어요" description="업무 상세에서 한 일을 적으면 여기에 모여요" />
            ) : (
                      ...
                      {list.map((w) => (
                        <div key={w.id} className="p-4 space-y-2 hover:bg-muted/20 transition-colors">
                          <div className="flex items-center gap-2.5 text-[12px] text-text-3">
                            ...
                            <Link href={`/personal/tasks/${w.taskId}`} className="font-medium text-foreground hover:text-blue hover:underline transition-colors">
                              {w.taskTitle}
                            </Link>
