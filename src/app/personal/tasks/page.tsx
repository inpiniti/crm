...
export default async function PersonalTasksPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const r = await attempt(() => Promise.all([listTasks({ ...filter, scope: "personal" }), listProjectOptions({ scope: "personal" }), listPersonOptions(), listAllTags("personal")]));
  if (!r.ok) return <SetupNotice error={r.error} />;
  const [tasks, projects, people, tags] = r.value;

  if (!hasSpecificFilter && tasks.length > 0) {
    redirect(`/personal/tasks/${tasks[0].id}`);
  }
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6">
      <PageHeader title="개인 업무 검색 및 필터" description={<span className="num">{tasks.length}건</span>} />
      <Suspense>
        <TaskFilters projects={projects} people={people} tags={tags} autoFocusSearch={one("focus") === "1"} />
      </Suspense>
      <TaskTable tasks={tasks} empty={filtered ? { title: "조건에 맞는 개인 업무가 없어요", description: "필터를 바꿔 보세요" } : undefined} basePath="/personal" />
    </div>
  );
}
