    ...
    <>
      ...
      {companies.length === 0 ? (
        ...
        <Panel className="divide-y divide-border">
          <div className="grid grid-cols-[1fr_200px_100px_100px] px-4 text-[12px] text-text-3">
            <div className="h-9 leading-9">회사</div>
            <div className="h-9 leading-9">기간</div>
            <div className="h-9 text-right leading-9">프로젝트</div>
            <div className="h-9 text-right leading-9">사람</div>
          </div>
          {companies.map((c) => (
            <Link key={c.id} href={`/companies/${c.id}`} className="row-hover grid h-11 grid-cols-[1fr_200px_100px_100px] items-center px-4 text-[13.5px]">
              ...
              <div className="num text-text-3">{c.joinedAt ? `${c.joinedAt} ~ ${c.leftAt ?? "현재"}` : "—"}</div>
              <div className="num text-right text-text-2">{c.projectCount}</div>
              <div className="num text-right text-text-2">{c.personCount}</div>
            </Link>
          ))}
        </Panel>
      )}
