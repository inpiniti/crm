    ...
    <>
      ...
      {people.length === 0 ? (
        ...
        <Panel className="divide-y divide-border">
          <div className="grid grid-cols-[180px_1fr_90px_90px_120px] px-4 text-[12px] text-text-3">
          ...
          {people.map((p) => (
            <Link key={p.id} href={`/people/${p.id}`} className="row-hover grid h-11 grid-cols-[180px_1fr_90px_90px_120px] items-center px-4 text-[13.5px]">
              <div className="truncate font-medium">{p.name}</div>
              <div className="truncate text-text-2">{[p.companyName, p.department, p.title].filter(Boolean).join(" · ") || <span className="text-text-3">—</span>}</div>
